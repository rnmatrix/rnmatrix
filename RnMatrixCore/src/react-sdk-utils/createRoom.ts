...
export default function createRoom(opts: IOpts): Promise<string | null> {
    ...
    switch (getAddressType(opts.dmUserId)) {
      ...
      case 'email':
        createOpts.invite_3pid = [
          {
            id_server: matrix.getClient().getIdentityServerUrl(true),
            medium: 'email',
  ...
  return client
    ...
    .finally(function () {
      console.log('opts.andView', opts.andView)
      if (opts.andView) {
        // TODO: navigate
        console.log('roomId=====>', roomId)
        console.log('roomId.roomId', roomId.roomId)
        opts.navigation.navigate('MessengerRoom', {
          roomId,
          room: null,
        });
        // dis.dispatch({
    ...
    .then(() => {
      if (opts.associatedWithCommunity) {
        return GroupStore.addRoomToGroup(
          opts.associatedWithCommunity,
          roomId,
          false,
        );
      }
    ...
    .filter((r) => {
      ...
      if (r && r.getMyMembership() === 'join') {
        ...
        const joinedMembers = members.filter((m) =>
          isJoinedOrNearlyJoined(m.membership),
        );
 ...
 */
export async function _waitForMember(
  client: MatrixClient,
  roomId: string,
  userId: string,
  opts = { timeout: 1500 },
 ...
 * can encrypt to.
 */
export async function canEncryptToAllUsers(
  client: MatrixClient,
  userIds: string[],
) {
  try {
...
export async function ensureDMExists(
  client: MatrixClient,
  userId: string,
): Promise<string> {
  ...
  if (existingDMRoom) {
    ...
    let encryption: boolean | undefined = undefined;
