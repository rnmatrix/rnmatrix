...
} from "@rn-matrix/ui/src/toasts/BulkUnverifiedSessionsToast";
import {
    hideToast as hideSetupEncryptionToast,
    Kind as SetupKind,
    showToast as showSetupEncryptionToast,
} from "@rn-matrix/ui/src/toasts/SetupEncryptionToast";
import {
    hideToast as hideUnverifiedSessionsToast,
    showToast as showUnverifiedSessionsToast,
} from "@rn-matrix/ui/src/toasts/UnverifiedSessionToast";
...
export default class DeviceListener {
  ...
  private keyBackupInfo: object | null = null;
  ...
  static makeShared() {
    DeviceListener.sharedInstance = new DeviceListener();
    return DeviceListener.sharedInstance;
  }

  start() {
    matrix.getClient().on('crypto.willUpdateDevices', this._onWillUpdateDevices);
    matrix.getClient().on('crypto.devicesUpdated', this._onDevicesUpdated);
    matrix.getClient().on('deviceVerificationChanged', this._onDeviceVerificationChanged);
    matrix.getClient().on('userTrustStatusChanged', this._onUserTrustStatusChanged);
    matrix.getClient().on('crossSigning.keysChanged', this._onCrossSingingKeysChanged);
    matrix.getClient().on('accountData', this._onAccountData);
    matrix.getClient().on('sync', this._onSync);
    matrix.getClient().on('RoomState.events', this._onRoomStateEvents);
  ...
  stop() {
    if (matrix.getClient()) {
      matrix.getClient().removeListener('crypto.willUpdateDevices', this._onWillUpdateDevices);
      matrix.getClient().removeListener('crypto.devicesUpdated', this._onDevicesUpdated);
      matrix
        .getClient()
        .removeListener('deviceVerificationChanged', this._onDeviceVerificationChanged);
      matrix.getClient().removeListener('userTrustStatusChanged', this._onUserTrustStatusChanged);
      matrix
        .getClient()
        .removeListener('crossSigning.keysChanged', this._onCrossSingingKeysChanged);
      matrix.getClient().removeListener('accountData', this._onAccountData);
      matrix.getClient().removeListener('sync', this._onSync);
      matrix.getClient().removeListener('RoomState.events', this._onRoomStateEvents);
    }
  ...
  _onDevicesUpdated = (users: string[]) => {
    if (!users.includes(matrix.getClient().getUserId())) return;
    this._recheck();
  };
  ...
  async _getKeyBackupInfo() {
    ...
    if (!this.keyBackupInfo || this.keyBackupFetchedAt! < now - KEY_BACKUP_POLL_INTERVAL) {
      this.keyBackupInfo = await matrix.getClient().getKeyBackupVersion();
    ...
    if (this.dismissedThisDeviceToast || allSystemsReady) {
      ...
      if (!cli.getCrossSigningId() && cli.getStoredCrossSigningForUser(cli.getUserId())) {
        ...
        if (backupInfo) {
          ...
          // FIXME? is isLoggedIn$ working fine?
          if (isSecureBackupRequired() && authService.isLoggedIn$()) {
