...
export default class MultiInviter {
  ...
  async _inviteToRoom(roomId, addr, ignoreProfile) {
    ...
    if (addrType === 'email') {
      return matrix.getClient().inviteByEmail(roomId, addr);
      ...
      if (
        !ignoreProfile &&
        SettingsStore.getValue('promptBeforeInviteUnknownUsers', this.roomId)
      ) {
        try {
          const profile = await matrix.getClient().getProfileInfo(addr);
          if (!profile) {
      ...
      return matrix.getClient().invite(roomId, addr);
    } else {
      ...
      doInvite
        ...
        .catch((err) => {
          ...
          if (err.errcode === 'M_FORBIDDEN') {
            ...
            errorText = _t('User %(userId)s is already in the room', {
              userId: address,
            });
            ...
            ['M_NOT_FOUND', 'M_USER_NOT_FOUND', 'RIOT.USER_NOT_FOUND'].includes(
              err.errcode,
            )
          ) {
            errorText = _t('User %(user_id)s does not exist', {
              user_id: address,
            });
          } else if (err.errcode === 'M_PROFILE_UNDISCLOSED') {
            errorText = _t('User %(user_id)s may or may not exist', {
              user_id: address,
            });
          } else if (err.errcode === 'M_PROFILE_NOT_FOUND' && !ignoreProfile) {
            // Invite without the profile check
            console.warn(
              `User ${address} does not have a profile - inviting anyways automatically`,
            );
            this._doInvite(address, true).then(resolve, reject);
          } else if (err.errcode === 'M_BAD_STATE') {
            errorText = _t(
              'The user must be unbanned before they can be invited.',
            );
          } else if (err.errcode === 'M_UNSUPPORTED_ROOM_VERSION') {
            errorText = _t(
              "The user's homeserver does not support the version of the room.",
            );
          } else {
  ...
  _inviteMore(nextIndex, ignoreProfile) {
    ...
    if (nextIndex === this.addrs.length) {
      ...
      if (Object.keys(this.errors).length > 0 && !this.groupId) {
        ...
        const unknownProfileUsers = Object.keys(this.errors).filter((a) =>
          unknownProfileErrors.includes(this.errors[a].errcode),
        );

        if (unknownProfileUsers.length > 0) {
          const inviteUnknowns = () => {
            const promises = unknownProfileUsers.map((u) =>
              this._doInvite(u, true),
            );
          ...
          const AskInviteAnywayDialog = sdk.getComponent(
            'dialogs.AskInviteAnywayDialog',
          );
          ...
          Modal.createTrackedDialog(
            ...
            AskInviteAnywayDialog,
            {
              unknownProfileUsers: unknownProfileUsers.map((u) => {
                return { userId: u, errorText: this.errors[u].errorText };
              }),
    ...
    this._doInvite(addr, ignoreProfile)
      .then(() => {
