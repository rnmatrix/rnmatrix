...
class GroupStore extends EventEmitter {
        ...
        this._resourceFetcher = {
            [this.STATE_KEY.Summary]: (groupId) => {
                return limitConcurrency(
                    () => matrix.getClient().getGroupSummary(groupId),
                );
            },
            [this.STATE_KEY.GroupRooms]: (groupId) => {
                return limitConcurrency(
                    () => matrix.getClient().getGroupRooms(groupId).then(parseRoomsResponse),
                );
            },
            [this.STATE_KEY.GroupMembers]: (groupId) => {
                return limitConcurrency(
                    () => matrix.getClient().getGroupUsers(groupId).then(parseMembersResponse),
                );
            },
            [this.STATE_KEY.GroupInvitedMembers]: (groupId) => {
                return limitConcurrency(
                    () => matrix.getClient().getGroupInvitedUsers(groupId).then(parseMembersResponse),
                );
            },
        };
    }
    ...
    inviteUserToGroup(groupId, userId) {
        return matrix.getClient().inviteUserToGroup(groupId, userId)
            .then(this._fetchResource.bind(this, this.STATE_KEY.GroupInvitedMembers, groupId));
    }

    acceptGroupInvite(groupId) {
        return matrix.getClient().acceptGroupInvite(groupId)
    ...
    joinGroup(groupId) {
        return matrix.getClient().joinGroup(groupId)
            // The user should now be able to access (personal) group settings
            .then(this._fetchResource.bind(this, this.STATE_KEY.Summary, groupId))
            // The user might be able to see more rooms now
            .then(this._fetchResource.bind(this, this.STATE_KEY.GroupRooms, groupId))
    ...
    leaveGroup(groupId) {
        ...
        return matrix.getClient().leaveGroup(groupId)
    ...
    setGroupPublicity(groupId, isPublished) {
        return matrix.getClient()
            .setGroupPublicity(groupId, isPublished)
            // .then(() => { FlairStore.invalidatePublicisedGroups(matrix.getClient().credentials.userId); })
            .then(this._fetchResource.bind(this, this.STATE_KEY.Summary, groupId));
    }
}
