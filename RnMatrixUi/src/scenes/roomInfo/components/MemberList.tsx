...
function rate_limited_func(fn, time) {
...
interface MemberListState {
  loading: boolean;
  members: any[];
  filteredJoinedMembers: any[];
  filteredInvitedMembers: any[];
...
export default class MemberList extends React.Component<MemberListProps, MemberListState> {
  private _showPresence: boolean = true;
  ...
  getMembersWithUser() {
    ...
    allMembers.forEach(function (member: any) {
  ...
  roomMembers() {
    ...
    const filteredAndSortedMembers = allMembers.filter((m: any) => {
  ...
  memberSort = (memberA, memberB) => {
    ...
    const nameA = (memberA.name[0] === '@' ? memberA.name.substr(1) : memberA.name).replace(
      SORT_REGEX,
      ''
    );
    const nameB = (memberB.name[0] === '@' ? memberB.name.substr(1) : memberB.name).replace(
      SORT_REGEX,
      ''
    );
  ...
  _onPending3pidInviteClick = (inviteEvent) => {
    console.log('_onPending3pidInviteClick');
    // dis.dispatch({
  ...
  _getPending3PidInvites() {
    ...
    const room = matrix.getClient().getRoom(this.props.roomId);
  ...
  //   this._makeMemberTiles(this.state.filteredJoinedMembers.slice(start, end));
  //
  ...
  renderItem = ({ item, index }) => {
    ...
    if (typeof item === 'string') {
      return (
        <View key={item} style={[theme.padding4x, theme.borderBottomHair, theme.borderPrimary]}>
          <Text style={[theme.fontXL, theme.fontSemibold]}>{item}</Text>
        </View>
      );
    }
    ...
    return (
      <UserListItem
        ...
        onPress={() =>
          item.user &&
          this.props.navigation.navigate('MessengerUserInfo', {
            user: item.user,
            room: this.room,
          })
        }
  ...
  handleEndReached = () => {
    this.setState({
      upperBound: this.state.upperBound + 50,
    });
  };

  private _actionSheetRef;
  ...
  render() {
    ...
    const members = this.state.filteredJoinedMembers;
    const invitedMembers = this.state.filteredInvitedMembers;
    ...
    return (
      <ErrorBoundary message={'ERROR'}>
        <FlatList
          ...
          data={[
            members && members.length > 0 ? 'Members' : undefined, // TODO: translate
            ...(members ? members : []),
            invitedMembers && invitedMembers.length > 0 ? 'Invites' : undefined, // TODO: translate
...
//
// enum RoomMemberActionSheetOption {
//   Cancel = 'Cancel',
//   Ban = 'Ban',
//   Kick = 'Kick',
// }
//
// const options = [
//   RoomMemberActionSheetOption.Cancel,
//   // RoomMemberActionSheetOption.Ban,
//   RoomMemberActionSheetOption.Kick,
// ];
//
